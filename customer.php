...
<div class="container mb-5">
  <div class="d-flex justify-content-between mb-3">
    ...
    <a href="tambahCustomer.php" class="btn btn-success">+ Tambah Customer</a>
  </div>
      ...
      <table class="table table-bordered table-hover align-middle">
        ...
        <tbody>
          <?php if(!empty($customer)): ?>
            <?php $i=1; foreach ($customer as $cst): ?>
              <tr>
                <td class="text-center"><?= $i++; ?></td>
                <td><?= htmlspecialchars($cst['nama']); ?></td>
                <td><?= htmlspecialchars($cst['alamat']); ?></td>
                <td><?= htmlspecialchars($cst['telpon']); ?></td>
                <td><?= htmlspecialchars($cst['email']); ?></td>
                <td class="text-center">
                  <!-- Update -->
                  <a href="ubahCustomer.php?id=<?= $cst['id']; ?>" class="btn btn-sm btn-primary">Update</a>
                  <!-- Hapus -->
                  <a href="hapusCustomer.php?id=<?= $cst['id']; ?>" 
                     class="btn btn-sm btn-danger" 
                     onclick="return confirm('Yakin hapus data customer ini?')">Hapus</a>
                </td>
              </tr>
            <?php endforeach; ?>
          <?php else: ?>
